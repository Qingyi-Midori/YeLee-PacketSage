...
5. 顺序保证：`run_started` 最先、`run_finished` 最后，两者之间至少一条 `tool_call_*` 或 `llm_round`。
6. **思考模式与缓存（v0.4 补）**：
...
22. **次要信息收进右侧栏**（v0.10 定稿，2026-09-23 实测通过）：`图例` / `抓包里有什么` /
    ...
    **教训（写进来防止再犯）**：`.composer-wrap` 是 `pointer-events: none`（为了让滚轮透给
    ...
    会话流里不再有任何"抓包里有什么"的折叠块。截图见
    `docs/ui/fixes-2026-09-23/`。
