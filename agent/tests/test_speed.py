...
def test_first_message_carries_facts_and_prefix_stays_stable(monkeypatch) -> None:
    ...
    provider.decide(
        TASK,
        [{"tool_name": "check_alerts", "args": {}, "result_summary": "ok"}],
        {},
        "看看",
        facts,
    )
    ...
    assert second["messages"][: len(first["messages"])] == first["messages"][: len(first["messages"])]
...
def test_run_mode_does_not_ask_for_a_fallback_answer() -> None:
    """`run` 是汇报结论，没写总结就不写——不替它编。"""
    provider = _AnsweringProvider([Decision(kind="final", findings=[], tokens_in=5, tokens_out=5)])
