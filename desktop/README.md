...
| 时间线 | 默认展开：引擎预取 → 工具调用 → 结论；工具节点可展开看入参/结果，结论就地挂 `tc_*` 锚点（点结论行的"证据 N"会跳过去并展开） |
...
| 泳道 | 每条左侧一个标签：`思考`（模型的 `reasoning_content`，DeepSeek 思考模式默认打开）/ `调用`（工具 + 参数 + 结果）/ `输出`（模型写正文）/ `结论` / `拒绝` |
...
| 对话历史 | 左栏「对话 · N」列出当前抓包下的每一轮（标题 = 你问的那句，点一下跳到那一轮）；按 `task_id` 存在本机 `localStorage`（引擎库没有对话文本，换机器就没了） |
...
界面四种状态的截图在 `../docs/ui/`（未开始 / 有结论 / 无异常 / 只有规则告警）——
**它们是旧版式的截图**，重构后待重拍。
